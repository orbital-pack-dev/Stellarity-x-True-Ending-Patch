...
execute if entity @e[type=ender_dragon,tag=!stellarity.ender_dragon] unless entity @e[type=minecraft:interaction,tag=ste_cos_crystal_shield,distance=..2,limit=1] run summon minecraft:interaction ~ ~-1 ~ {width:3.0f,height:4.0f,response:0b,Tags:["ste_cos_crystal_shield"]}
