...
execute positioned ~0.5 ~0.25 ~0.5 run function ste_cos:egg/egg_ambient

# Волны и вспышки (оркестрация)
execute if score @s ste_cos.flags matches 40 positioned ~0.5 ~0.25 ~0.5 run function ste_cos:egg/egg_wave
execute if score @s ste_cos.flags matches 40.. run scoreboard players set @s ste_cos.flags 0

# Редкие искры Enchant
execute if score @s ste_cos.flags matches 3 positioned ~0.5 ~0.25 ~0.5 run particle enchant ~ ~ ~ 0 0 0 1 2 force

# Оркестратор пульса
# ВАЖНО: раньше здесь не было "positioned ~0.5 ~0.25 ~0.5" — pulse_bright/pulse_dark
...
execute if score @s ste_cos.flags matches 0 positioned ~0.5 ~0.25 ~0.5 run function ste_cos:egg/egg_pulse_trigger
