...
execute if entity @a[distance=..20,gamemode=!spectator,gamemode=!creative] run data modify entity @e[type=phantom,tag=ste_cos_guard,distance=..6,limit=1] NoAI set value 0b

# Игрока нет → AI выкл + орбита
execute unless entity @a[distance=..20,gamemode=!spectator,gamemode=!creative] run data modify entity @e[type=phantom,tag=ste_cos_guard,distance=..6,limit=1] NoAI set value 1b
