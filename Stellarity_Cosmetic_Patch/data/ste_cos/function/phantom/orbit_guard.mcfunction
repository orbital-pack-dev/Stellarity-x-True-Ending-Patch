...
execute at @s rotated as @s positioned ~ ~3 ~ as @e[type=phantom,tag=ste_cos_guard,distance=..64] if score @s ste_cos.id = #current_id ste_cos.flags run tp @s ^ ^ ^7

# Зафиксировать взгляд фантома (чтобы смотрел на кристалл)
execute at @s positioned ~ ~3 ~ as @e[type=phantom,tag=ste_cos_guard,distance=..64] if score @s ste_cos.id = #current_id ste_cos.flags at @s run tp @s ~ ~ ~ facing ^ ^ ^-1
