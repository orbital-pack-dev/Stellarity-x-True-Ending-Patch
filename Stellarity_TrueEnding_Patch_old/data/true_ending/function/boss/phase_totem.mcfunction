# =====================================================================
# PATCH OVERRIDE: true_ending:boss/phase_totem
# Оригинальный файл: tru.e-ending-1.1.4d/data/true_ending/function/boss/phase_totem.mcfunction
#
# BUG FIX #2 - LANDING Y-OFFSET:
#   Строка 13 оригинала: positioned over motion_blocking run tp @s ~ ~2 ~
#   Алтарь Stellarity выше vanilla портала на ~5 блоков.
#   Меняем ~2 → ~7, чтобы дракон садился поверх алтаря, а не внутрь блоков.
# =====================================================================
...
# BUG FIX #2: ~2 → ~7 (поднять точку приземления на алтарь Stellarity)
execute if score @s trueEnding_bosstime matches 3001 positioned over motion_blocking run tp @s ~ ~7 ~
...
execute if score @s trueEnding_bosstime matches 3081 run particle flash{color:[1.0, 1.0, 1.0, 1.0]} ~ ~ ~ 0 0 0 0 5 force @a[distance=..128]
