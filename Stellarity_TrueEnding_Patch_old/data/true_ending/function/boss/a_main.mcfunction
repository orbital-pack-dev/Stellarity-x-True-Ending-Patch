...
# Восстановлен ПОЛНЫЙ оригинальный босс-луп True Ending из
#   tru.e-ending-1.1.4d/data/true_ending/function/boss/a_main.mcfunction
#
# ЕДИНСТВЕННОЕ ИЗМЕНЕНИЕ vs оригинал:
#   Строка снятия Invulnerable (ниже в блоке #invulnerabity) теперь
#   срабатывает ТОЛЬКО когда уничтожены все кристаллы
#   (if score $crystals_gone ste_te_flags matches 1).
#   Это гарантирует, что пока кристаллы живы, TE не снимает щит.
# =====================================================================
...
execute unless score @s[tag=!trueEnding_inattack] trueEnding_bosstime matches 3000.. unless score @s trueEnding_health_percent matches ..100 if score $crystals_gone ste_te_flags matches 1 run data modify entity @s Invulnerable set value 0b
