...
# Телепорт к рандомному кристальному столбу, но теперь это относительный телепорт без жесткого Y
execute as @e[type=end_crystal,sort=random,limit=1] at @s run tp @e[type=ender_dragon,tag=stellarity.ender_dragon,limit=1] ~ ~ ~

# Позиционируем на 1 блок выше, чтобы не застрять в блоке
execute positioned ~ ~-1 ~ unless block ~ ~-1 ~ air run tp @s ~ ~1 ~
...
# Отбрасывание
execute at @s as @a[distance=..30] run data merge entity @s {Motion:[2.0,1.0,2.0]}
