...
# таймер волн опасности у портала

execute positioned 0 64 0 unless entity @e[type=ender_dragon,distance=..25] run scoreboard players add #wave_timer ste_cos.flags 1
...
execute positioned 0 64 0 if entity @e[type=ender_dragon,distance=..25] run scoreboard players set #wave_timer ste_cos.flags 0
execute positioned 0 64 0 if entity @e[type=ender_dragon,distance=..25] run kill @e[type=area_effect_cloud,tag=ste_cos_portal_wave]
