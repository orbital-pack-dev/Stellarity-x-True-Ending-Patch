...
# Когда подлетает в радиус 8 блоков от 0 65 0 — начинается анимированный взлет
execute if score #final_breath_state ste_cos.flags matches 1 as @e[type=ender_dragon,tag=ste_cos.final_breath_guided,limit=1] at @s positioned 0 65 0 if entity @s[distance=..8] run function ste_cos:final_breath/start
