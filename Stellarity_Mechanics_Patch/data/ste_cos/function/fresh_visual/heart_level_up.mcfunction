...
# фиолетовая вспышка и втягивание космической энергии при подключении кристалла
...
# громкие звуки перезвона, магии и резонанса (слышны на всем острове, медленный темп)
execute as @e[type=marker,tag=ste_cos.respawn_heart,limit=1] at @s run playsound block.amethyst_block.chime hostile @a ~ ~ ~ 48.0 0.70
execute as @e[type=marker,tag=ste_cos.respawn_heart,limit=1] at @s run playsound block.amethyst_block.resonate hostile @a ~ ~ ~ 48.0 0.60
execute as @e[type=marker,tag=ste_cos.respawn_heart,limit=1] at @s run playsound entity.illusioner.mirror_move hostile @a ~ ~ ~ 48.0 0.65
