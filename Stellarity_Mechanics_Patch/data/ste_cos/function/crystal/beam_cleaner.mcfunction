...
# очистка кристаллов башен (дистанция 25+ блоков от центра)
...
# проверка башен
execute in minecraft:the_end positioned 0 65 0 unless entity @e[type=end_crystal,distance=25..300,limit=1] run scoreboard players set #crystal_despawn_timer ste_cos.timer 0
execute in minecraft:the_end positioned 0 65 0 unless entity @e[type=end_crystal,distance=25..300,limit=1] run return 0

# лучи на портал (только с настоящих башен)
execute in minecraft:the_end positioned 0 65 0 as @e[type=end_crystal,distance=25..300] unless data entity @s {beam_target:[I;0,65,0]} run data modify entity @s beam_target set value [I;0,65,0]
...
execute if score #crystal_despawn_timer ste_cos.timer matches 0 in minecraft:the_end positioned 0 65 0 as @e[type=end_crystal,distance=25..300,limit=1,sort=random] at @s run function ste_cos:crystal/despawn_one
