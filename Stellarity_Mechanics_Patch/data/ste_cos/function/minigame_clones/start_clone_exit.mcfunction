...
# посадка копий на скоростные армор-стенды, разлетающиеся по противоположным C-дугам за остров

# Копия 1: летит по C-дуге в сторону +X, +Z
...
execute as @e[type=ender_dragon,tag=ste_cos.clone_1,limit=1] at @s run ride @s mount @e[type=armor_stand,tag=ste_cos.exit_1,limit=1,sort=nearest]
execute as @e[type=armor_stand,tag=ste_cos.exit_1] facing 100 90 100 run tp @s ~ ~ ~ ~ ~

# Копия 2: летит по C-дуге в противоположную сторону -X, -Z
