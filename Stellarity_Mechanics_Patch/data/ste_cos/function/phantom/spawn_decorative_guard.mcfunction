# ste_cos:phantom/spawn_decorative_guard
# декоративный страж

# защита от дублирования
execute if entity @e[type=area_effect_cloud,tag=ste_cos_guard_marker,distance=..6,limit=1] run return 0
execute if entity @e[type=phantom,tag=ste_cos_guard,distance=..20,limit=1] run return 0

# шаг для параметров орбиты
...
# уникальный номер стража
scoreboard players add #guard_id ste_cos.flags 1

# маркер привязки к кристаллу
