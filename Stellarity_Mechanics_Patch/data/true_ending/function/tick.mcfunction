...
#respawn animation (строго по 4 кристаллам на постаментах алтаря, радиус 4.5 блока)
execute if score respawn_ender_dragon trueEnding_storage matches 0.. in the_end positioned 0 62 0 unless entity @e[type=end_crystal,distance=..4.5,tag=stellarity.respawn_crystal,limit=1] run scoreboard players reset respawn_ender_dragon trueEnding_storage
execute unless score respawn_ender_dragon trueEnding_storage matches 0.. in the_end positioned 0 62 0 if score #respawn_crystal_count stellarity.misc matches 4 if data entity @e[type=end_crystal,distance=..4.5,tag=stellarity.respawn_crystal,limit=1] beam_target run scoreboard players add respawn_ender_dragon trueEnding_storage 0
execute if score respawn_ender_dragon trueEnding_storage matches 0.. in the_end positioned 0 62 0 if score #respawn_crystal_count stellarity.misc matches 4 if entity @p[distance=..128] run function true_ending:respawning/a_main
