...
// ---------------- CONFIG ----------------
const float HALO_RADIUS = 0.14;
const float VEIL_RADIUS = 0.48;

const float HALO_INTENSITY = 0.42;
const float VEIL_INTENSITY = 0.20;

const vec3 GLARE_COLOR = vec3(123.0/255.0, 158.0/255.0, 216.0/255.0);

// Horizontal streaks
const float STREAK_INTENSITY = 0.10;

const float STREAK_LENGTH_1 = 0.045;
const float STREAK_THICKNESS_1 = 0.006;
const float STREAK_Y_OFFSET_1 = 0.014;

const float STREAK_LENGTH_2 = 0.020;
const float STREAK_THICKNESS_2 = 0.006;
const float STREAK_Y_OFFSET_2 = -0.016;
const float STREAK_SECOND_INTENSITY = 0.75;
...
// Пикселизация всех glare-слоёв: halo / veil / streak / ghosts.
const float PIXEL_SIZE = 0.0035;
...
const float VIS_RADIUS_PX = 96.0;

const float MOON_DISC_RADIUS_PX = 96.0;
const float EDGE_PRESERVE = 1.0;

// Фейд по краям экрана
const float SCREEN_EDGE_FADE_X = 0.10;
const float SCREEN_EDGE_FADE_Y = 0.12;
...
vec3 getLightScreenPos(float angle, out bool isBehind, out float screenFade) {
    vec3 lightDirWorld = normalize(vec3(
        cos(angle),
        sin(angle),
        0.0
    ));
    ...
    vec3 lightPosWorld = camPos - lightDirWorld * 1000.0;
...
void getLightWeights(float angle, out float sunWeight, out float moonWeight) {
    ...
    if (t < TRANSITION_WIDTH) {
        ...
        sunWeight = 1.0 - (t - (PI - TRANSITION_WIDTH)) / (2.0 * TRANSITION_WIDTH);
    } else if (t < 2.0 * PI - TRANSITION_WIDTH) {
        sunWeight = 0.0;
    } else {
        sunWeight = (t - (2.0 * PI - TRANSITION_WIDTH)) / TRANSITION_WIDTH;
    }
...
vec2 getMoonVisibility(vec2 lightUV) {
    ...
    vec2 discR = px * MOON_DISC_RADIUS_PX;
...
void main() {
    ...
    float moonFadeIn = smoothstep(radians(0.0), radians(6.0), PolySunAngle);
    float moonFadeOut = 1.0 - smoothstep(radians(174.0), radians(180.0), PolySunAngle);
    moonWeight *= moonFadeIn * moonFadeOut;

    const float PI = 3.14159;
    float moonAngle = PolySunAngle + PI;

    bool isBehind;
    float screenFade;
    vec3 lightData = getLightScreenPos(moonAngle, isBehind, screenFade);

    if (isBehind || moonWeight <= 0.0 || screenFade <= 0.0) {
        fragColor = base;
        return;
    }

    vec2 lightUV = lightData.xy;
    vec2 vis = getMoonVisibility(lightUV);

    float moonVisibleAvg = vis.x;
    float moonVisibleEdge = vis.y;

    float lookFactor = smoothstep(0.78, 0.14, distance(lightUV, vec2(0.5, 0.5)));
    float rawStrength = moonWeight * screenFade * lookFactor;

    float edgeSupport = max(
        moonVisibleAvg,
        moonVisibleEdge * EDGE_PRESERVE + moonVisibleAvg * (1.0 - EDGE_PRESERVE)
    );

    float haloStrength = smoothstep(0.03, 0.18, rawStrength * edgeSupport);
    float veilStrength = smoothstep(0.02, 0.14, rawStrength * edgeSupport);
    float streakStrength = smoothstep(0.02, 0.18, rawStrength * edgeSupport);
    float ghostStrength = smoothstep(0.04, 0.20, rawStrength * edgeSupport);
    ...
    // используют одну и ту же блочную UV-сетку.
    ...
    float washout = veil * VEIL_INTENSITY * veilStrength * (0.20 + 0.35 * sceneMask);

    color += localGlow;
    color = mix(color, color + GLARE_COLOR * 0.18, washout);
